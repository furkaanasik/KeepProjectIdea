...
describe('runClaude', () => {
  ...
  it('respects CLAUDE_TIMEOUT_MS env var when timeoutMs not passed', async () => {
    const { spawnFn, lastChild } = makeSpawn(() =>
      makeFakeChild({ neverClose: true }),
    );
    ...
    try {
      ...
      if (original === undefined) {
        ...
        process.env['CLAUDE_TIMEOUT_MS'] = original;
      }
    }

    expect(caught).toBeInstanceOf(ClaudeRunError);
    expect((caught as ClaudeRunError).message).toMatch(/timed out after 50ms/i);
    expect(lastChild()?.killed).toBe(true);
  });

  it('explicit timeoutMs overrides CLAUDE_TIMEOUT_MS env var', async () => {
    const { spawnFn, lastChild } = makeSpawn(() =>
      makeFakeChild({ neverClose: true }),
    );

    const original = process.env['CLAUDE_TIMEOUT_MS'];
    process.env['CLAUDE_TIMEOUT_MS'] = '999999';
    let caught: unknown;
    try {
      await runClaude({ prompt: 'hi', timeoutMs: 40, spawnFn });
    } catch (err) {
      caught = err;
    } finally {
      if (original === undefined) {
    ...
    expect(caught).toBeInstanceOf(ClaudeRunError);
    expect((caught as ClaudeRunError).message).toMatch(/timed out after 40ms/i);
    expect(lastChild()?.killed).toBe(true);
  });
